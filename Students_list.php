...
$sql = "SELECT * FROM students LIMIT $limitStart, $rowsPerPage";
...
<body class="bg-fixed bg-no-repeat bg-cover bg-center bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-[#f3f4f5] via-[#e0e8ed] to-[#d0dde6]">
  ...
  <div >
        ...
        <aside>
          ...
          >
            ...
            <div name="program" id="dropdown" class="items-center w-[160px] h-[40px] flex px-5 py-0.5 rounded-[40px] 
              ...
              <div id="programDropdown" class="dropdown-content">
                <!--a href="students_list.php?column=program&value=">Link</a>
                <a href="students_list.php?column=program&value=">Link</a>
                <a href="students_list.php?column=program&value=">Link</a-->
                <?php 
          ...
          if ($result-> num_rows > 0) {
              while ($row =  $result-> fetch_assoc()){
                //$program = $row['program'];
                echo "<a href='students_list.php?column=program&progvalue=". $row["program"]."'>". $row["program"]."</a>";
        ...
        <!--/form-->
          ...
          $sql = "SELECT DISTINCT year_lvl from students";
          $result = $conn-> query($sql);

          if ($result-> num_rows > 0) {
              while ($row =  $result-> fetch_assoc()){
                //$program = $row['program'];
                echo "<a href='students_list.php?column=year_lvl&yrsec=". $row["year_lvl"]."'>". $row["year_lvl"]."</a>";
                //echo "<option value= '". $row["program"]."'>". $row["program"]."</a>";
              }
          }
          else{
          ...
          ?>
            ...
            <!--a href="#" class="justify-center items-center w-[125px] h-[40px] flex px-5 py-2 rounded-[40px] 
              ...
              <div id="sortDropdown" class="dropdown-content">
                <a href="students_list.php?column=last_n&order=asc">Ascending</a>
                <a href="students_list.php?column=last_n&order=desc">Descending</a>
                <a href="students_list.php">Default</a>
              </div>  
            </div>
        </div>
        </div>
        ...
        <div class="flex flex-col items-center">
            ...
            <div class="self-center flex w-[100%] max-w-full max-md:flex-wrap justify-evenly mb-5">
              <table>
                ...
                <th class="text-orange-950 font-semibold leading-6">Programs</th>
          ...
          <?php  
                  if (isset($_GET['order'])){
                    $sql = "SELECT stud_num, year_lvl, program, last_n, first_n, mid_n, contactno, email from students ORDER BY last_n $sort_order LIMIT $limitStart, $rowsPerPage";
                  }
                  else if (isset($_GET['progvalue'])){
                    $sql = "SELECT stud_num, year_lvl, program, last_n, first_n, mid_n, contactno, email from students WHERE program = '$program' LIMIT $limitStart, $rowsPerPage";
                  }
                  else if (isset($_GET['yrsec'])){
                    $sql = "SELECT stud_num, year_lvl, program, last_n, first_n, mid_n, contactno, email from students WHERE year_lvl = '$yrsec' LIMIT $limitStart, $rowsPerPage";
                  }
                  else if (isset($_GET['order']) && isset($_GET['progvalue'])) {
                    $sql = "SELECT stud_num, year_lvl, program, last_n, first_n, mid_n, contactno, email from students WHERE program = '$program' ORDER BY reqtype $sort_order LIMIT $limitStart, $rowsPerPage";
                  }
                  else {
                    $sql = "SELECT stud_num, year_lvl, program, last_n, first_n, mid_n, contactno, email from students LIMIT $limitStart, $rowsPerPage";
                  }
            ...
            <td><?php echo $row["last_n"] .", ". $row["first_n"] ." ". $row["mid_n"]; ?></td> <!-- dto jackie-->
...
<div class="flex justify-center mt-2 mb-2">
    <?php
    $totalRows = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM students"));
    $totalPages = ceil($totalRows / $rowsPerPage);

    if(isset($_GET['order'])) {
      // Previous page button
      if ($currentPage > 1) {
        echo "<a href='enrollees.php?order=".$sort_order."&page=" . ($currentPage - 1) . "' class='mx-2 px-4 py-2 bg-stone-500 text-white rounded-3xl'>Previous</a>";
    }

    // Page numbers
    for ($i = 1; $i <= $totalPages; $i++) {
        echo "<a href='enrollees.php?order=".$sort_order."&page=$i' class='mx-2 px-4 py-2 bg-stone-500 text-white rounded-xl'>$i</a>";
    }

    // Next page button
    if ($currentPage < $totalPages) {
        echo "<a href='enrollees.php?order=".$sort_order."&page=" . ($currentPage + 1) . "' class='mx-1 px-2 py-1 bg-stone-500 text-white rounded-full'>Next</a>";
    }
    }
    else if (isset($_GET['progvalue'])) {
      // Previous page button
      if ($currentPage > 1) {
        echo "<a href='enrollees.php?progvalue=".$program."&page=" . ($currentPage - 1) . "' class='mx-1 px-2 py-1 bg-stone-500 text-white rounded-full'>Previous</a>";
    }

    // Page numbers
    for ($i = 1; $i <= $totalPages; $i++) {
        echo "<a href='enrollees.php?progvalue=".$program."&page=$i' class='mx-2 px-4 py-2 bg-stone-500 text-white rounded-3xl'>$i</a>";
    }

    // Next page button
    if ($currentPage < $totalPages) {
        echo "<a href='enrollees.php?progvalue=".$program."&page=" . ($currentPage + 1) . "' class='mx-2 px-4 py-2 bg-stone-500 text-white rounded-3xl'>Next</a>";
    }
    }
    else {
      // Previous page button
