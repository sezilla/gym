  ...
  <div class="">
        ...
        <aside>
          ...
          >
            ...
            <!--form action="enrollees.php" method="get"-->
            <div name="program" id="dropdown" class="items-center w-[160px] h-[40px] flex px-5 py-0.5 rounded-[40px] 
              ...
              <div id="programDropdown" class="dropdown-content">
                <!--a href="enrollees.php?column=program&value=">Link</a>
                <a href="enrollees.php?column=program&value=">Link</a>
                <a href="enrollees.php?column=program&value=">Link</a-->
                <?php 
          
          $sql = "SELECT DISTINCT program from enrollees";
          $result = $conn-> query($sql);

          if ($result-> num_rows > 0) {
              while ($row =  $result-> fetch_assoc()){
                //$program = $row['program'];
                echo "<a href='enrollees.php?column=program&progvalue=". $row["program"]."'>". $row["program"]."</a>";
        ...
        <!--/form-->
          ...
          $sql = "SELECT DISTINCT year_lvl from enrollees";
          $result = $conn-> query($sql);

          if ($result-> num_rows > 0) {
              while ($row =  $result-> fetch_assoc()){
                //$program = $row['program'];
                echo "<a href='enrollees.php?column=year_lvl&yrsec=". $row["year_lvl"]."'>". $row["year_lvl"]."</a>";
                //echo "<option value= '". $row["program"]."'>". $row["program"]."</a>";
              }
          }
          else{
          ...
          ?>
            ...
            <!--a href="#" class="justify-center items-center w-[125px] h-[40px] flex px-5 py-2 rounded-[40px] 
              ...
              <div id="sortDropdown" class="dropdown-content">
                <a href="enrollees.php?column=last_n&order=asc">Ascending</a>
                <a href="enrollees.php?column=last_n&order=desc">Descending</a>
                <a href="enrollees.php">Default</a>
              </div>  
            </div>
        </div>
        </div>
...
<!--SHOWDATA-->
            ...
            <td><?php echo $row["last_n"] .", ". $row["first_n"] ." ". $row["mid_n"]; ?></td>
            <td><?php echo $row["contactno"]; ?></td>
            <td><?php echo $row["email"]; ?></td>
            <td>
            <form href="enrollees.php" action="actionbutton.php" method="POST">
                <button name="accept" value="<?=$row['stud_num'];?>" class='bg-stone-500 text-white text-sm leading-5 font-medium rounded-full px-2 py-2 mr-2'>
                ...
                <button name="reject" value="<?=$row['stud_num'];?>" class='bg-stone-500 text-white text-sm leading-5 font-medium rounded-3xl px-2 py-2 mr-2'>
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" style="fill: #ffffff;transform: ;msFilter:;">
...
<div class="flex justify-center mt-4 mb-4">
    ...
    for ($i = 1; $i <= $totalPages; $i++) {
        echo "<a href='enrollees.php?order=".$sort_order."&page=$i' class='mx-1 px-2 py-1 bg-stone-500 text-white rounded-full'>$i</a>";
    }
    ...
    // Page numbers
    for ($i = 1; $i <= $totalPages; $i++) {
        echo "<a href='enrollees.php?progvalue=".$program."&page=$i' class='mx-1 px-2 py-1 bg-stone-500 text-white rounded-3xl'>$i</a>";
    }

    // Next page button
    if ($currentPage < $totalPages) {
        echo "<a href='enrollees.php?progvalue=".$program."&page=" . ($currentPage + 1) . "' class='mx-1 px-2 py-1 bg-stone-500 text-white rounded-3xl'>Next</a>";
    }
    }
    else {
      // Previous page button
      if ($currentPage > 1) {
        echo "<a href='enrollees.php?page=" . ($currentPage - 1) . "' class='mx-1 px-2 py-1 bg-stone-500 text-white rounded-3xl'>Previous</a>";
    }

    // Page numbers
    for ($i = 1; $i <= $totalPages; $i++) {
        echo "<a href='enrollees.php?page=$i' class='mx-1 px-2 py-1 bg-stone-500 text-white rounded-3xl'>$i</a>";
    }

    // Next page button
    if ($currentPage < $totalPages) {
        echo "<a href='enrollees.php?page=" . ($currentPage + 1) . "' class='mx-1 px-2 py-1 bg-stone-500 text-white rounded-3xl'>Next</a>";
    }}
    ?>
</div>
<!--PAGE BUTTON END-->
